-- $hashed_password$ is a placeholder for the hashed password
INSERT INTO users (id, username, host, protocol, hashed_password, display_name, profile, icon)
VALUES ('1', 'mock', 'localhost', 'local', '$hashed_password$', 'mock user', 'This is mock user', 'https://placehold.jp/150x150.png');
INSERT INTO users (id, username, host, protocol, hashed_password, display_name, profile, icon)
VALUES ('2', 'mock', 'sechack365-dev.claustra01.net', 'local', '$hashed_password$', 'mock user', 'This is mock user', 'https://placehold.jp/150x150.png');
INSERT INTO ap_user_identifiers (user_id, public_key, private_key)
VALUES ('1', '-----BEGIN PUBLIC KEY-----

AwIDAQAB
-----END PUBLIC KEY-----
', '-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----
');
INSERT INTO ap_user_identifiers (user_id, public_key, private_key)
VALUES ('2', '-----BEGIN PUBLIC KEY-----

AwIDAQAB
-----END PUBLIC KEY-----
', '-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----
');